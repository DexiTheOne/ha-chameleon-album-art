...
- **Only Interesting Colors** now rejects only pure black. Dark colors, gray, muted colors, white, and repeated hues remain eligible in album art and image scenes. Entirely black images are detected before palette quantization; if no colors remain, the lights stay unchanged. Mostly white images retain the existing white/accent assignment behavior.
