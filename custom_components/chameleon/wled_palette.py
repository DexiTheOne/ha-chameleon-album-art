...
async def send_wled_palette(hass: HomeAssistant, entity_id: str, colors: list[RGBColor], offset: int) -> None:
    """Update the segment represented by a configured WLED light."""
    ...
    if entry is None or entry.domain != "wled":
        return
    try:
        segment_id = int(entity.unique_id.rsplit("_", 1)[1])
    except (AttributeError, ValueError):
        _LOGGER.warning("Cannot identify WLED segment for %s", entity_id)
        return
    ...
    try:
        ...
        async with session.post(
            ...
            json={"seg": [{"id": segment_id, "col": [list(color) for color in palette]}]},
            timeout=5,
        ) as response:
