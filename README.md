...
- Turn on **Send Palette information to WLED** to give each configured WLED device three distinct scene colors through its JSON API. Chameleon applies them to every segment on that device and selects WLED's Palette effect with its Colors Only palette. The switch starts off and does not affect non-WLED lights or manual colors. Turning it off stops future WLED palette requests; it does not restore the device's previous WLED effect.
