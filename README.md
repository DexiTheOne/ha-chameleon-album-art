...
- Turn on **Send Palette information to WLED** to send three distinct colors extracted from an image to every segment of a configured WLED device. Chameleon sends them only when the image has at least three distinct colors and every segment already uses a color-slot palette with a palette-aware effect. It preserves the device's selected effect and palette. Otherwise the light receives its ordinary Chameleon color. The switch starts off and does not affect non-WLED lights or manual colors.
