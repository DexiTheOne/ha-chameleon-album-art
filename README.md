...
- Choose **WLED** in Transition Style to use each configured WLED device's native scene transition. The WLED Transition Style control offers Fade, Fairy Dust, Swipe Right/Left, Push Right/Left, Outside In, and Inside Out; the Transition slider sets its duration. Devices with a one-LED segment use Fade for all their segments because WLED applies transition style per device; they keep the same duration. All configured WLED segments receive their assigned image colors together in one request per device. Non-WLED lights receive their new colors immediately in this mode. Continuous Chameleon animation is deferred while WLED mode is selected.
- The transition duration and both style controls are saved across Home Assistant restarts. Turning Chameleon off uses the selected fade duration; in WLED mode it also uses the selected native style. Turning it on applies the selected scene with that duration. Repeated Random selections choose a different scene when more than one is available.
